...
DROP POLICY IF EXISTS "Profiles created via trigger only" ON profiles;

-- Drop all course policies
DROP POLICY IF EXISTS "Anyone can read courses" ON courses;
DROP POLICY IF EXISTS "Only admin can create courses" ON courses;
...
DROP POLICY IF EXISTS "Only admin can create enrollments" ON enrollments;
DROP POLICY IF EXISTS "Only admin can update enrollments" ON enrollments;
DROP POLICY IF EXISTS "Only admin can delete enrollments" ON enrollments;

-- Drop all material policies
DROP POLICY IF EXISTS "Students can read materials for enrolled courses" ON materials;
DROP POLICY IF EXISTS "Admins can read all materials" ON materials;
DROP POLICY IF EXISTS "Only admin can create materials" ON materials;
DROP POLICY IF EXISTS "Only admin can update materials" ON materials;
DROP POLICY IF EXISTS "Only admin can delete materials" ON materials;

-- Drop all announcement policies
DROP POLICY IF EXISTS "Students can read course and global announcements" ON announcements;
DROP POLICY IF EXISTS "Admins can read all announcements" ON announcements;
DROP POLICY IF EXISTS "Only admin can create announcements" ON announcements;
...
-- This is the primary policy that allows users to see their own data
CREATE POLICY "Users can read their own profile"
  ON profiles FOR SELECT
  USING (auth.uid() = id);

-- Users can update their own profile
-- They cannot change their role (enforced by checking current role matches)
